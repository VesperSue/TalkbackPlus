...
import static pcg.talkbackplus.Utility.CMD_HIDE_OVERLAY;
import static pcg.talkbackplus.Utility.CMD_SHOW_OVERLAY;

/**
 * Created by kinnplh on 2017/10/25.
 */

public class TalkBackPlusService extends AccessibilityService
    ...
    private static TalkBackPlusService sInstance = null;
    ...
    private final static String LOGTAG = "TalkBackPlusService";
    ...
    private TalkBackPlusRefreshThread refreshThread;
    private TalkBackPlusEventThread eventThread;
    ...
    public void onCreate() {
        ...
        // 初始化线程
        refreshThread = new TalkBackPlusRefreshThread(this);
        eventThread = new TalkBackPlusEventThread(this);
    ...
    public void disableTalkBack() {
        ...
        TalkBackPlusService.getInstance().getSpeechController().speak(
    ...
    public static int getServiceState() {
        final TalkBackPlusService service = getInstance();
        if (service == null) {
    ...
    /**
     * Whether the current TalkBackPlusService instance is running and initialized.
     ...
     */
    public static TalkBackPlusService getInstance() {
        return sInstance;
    }
    ...
    private void initializeInfrastructure() {
        ...
        if (mKeyComboManager != null) {
            ...
            // Search mode should receive key combos immediately after the TalkBackPlusService.
...
//            final Intent tutorial = new Intent(this, SettingActivity.class);
//            tutorial.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//            startActivity(tutorial);
                ...
                public boolean onComboPerformed(int id) {
                    switch (id) {
                        ...
                        case KeyComboManager.ACTION_BACK:
                            TalkBackPlusService.this.performGlobalAction(GLOBAL_ACTION_BACK);
                            return true;
                        case KeyComboManager.ACTION_HOME:
                            TalkBackPlusService.this.performGlobalAction(GLOBAL_ACTION_HOME);
                            return true;
                        case KeyComboManager.ACTION_NOTIFICATION:
                            TalkBackPlusService.this.performGlobalAction(GLOBAL_ACTION_NOTIFICATIONS);
                            return true;
                        case KeyComboManager.ACTION_RECENTS:
                            TalkBackPlusService.this.performGlobalAction(GLOBAL_ACTION_RECENTS);
                            return true;
     ...
     * <p>
     * {@link TalkBackPlusService#addServiceStateListener(ServiceStateListener)}
     * {@link TalkBackPlusService#removeServiceStateListener(ServiceStateListener)}
     * {@link TalkBackPlusService#SERVICE_STATE_INACTIVE}
     * {@link TalkBackPlusService#SERVICE_STATE_ACTIVE}
     * {@link TalkBackPlusService#SERVICE_STATE_SUSPENDED}
     */
    ...
    public static TalkBackPlusService self;
    ...
    public void onServiceConnected() {
        ...
        mSpeechController.speak(getString(R.string.blindhelper_on),
                SpeechController.QUEUE_MODE_UNINTERRUPTIBLE, 0, null);

        talkBackPlusOnConnected();
    }

    private void talkBackPlusOnConnected()
    {
        suspendTalkBack();
        overlayController = new OverlayController(self,eventThread);
        handler = new Handler(){
            @Override
            public void handleMessage(Message msg) {
                switch (msg.arg1){
                    case CMD_HIDE_OVERLAY:
                        Log.i("info", "handleMessage: onAccessibilityEvent get hide overlay cmd");
                        hideOverlay();
                        break;
                    case CMD_SHOW_OVERLAY:
                        Log.i("info", "handleMessage: onAccessibilityEvent get show overlay cmd");
                        showOverlay();
                        break;
                    default:
                        break;
    ...
    public void onAccessibilityEvent(final AccessibilityEvent accessibilityEvent) {
        ...
        // todo 如果是和界面刷新相关的 AccessibilityEvent，并且页面刷新的线程被挂起，则将其唤醒
        if(accessibilityEvent.getEventType() == AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED
            ...
            new Thread("accessibility refresh"){
                @Override
                public void run() {
                    synchronized (refreshThread.REFRESH_THREAD_WAIT_VAR) {
                        refreshThread.countRefreshThread += 1;
                        refreshThread.REFRESH_THREAD_WAIT_VAR.notify();
                    }
                }
            }.start();
        }
        ...
        if(isServiceActive())
            mAccessibilityEventProcessor.onAccessibilityEvent(accessibilityEvent);
    }
