...
public class NodeSpeechRuleProcessor {
    ...
    public CharSequence getDescriptionForTree(AccessibilityNodeInfoCompat announcedNode,
        ...
        appendDescriptionForTree(announcedNode, builder1, event, source, visitedNodes);
        String tmpString = builder1.toString();
        tmpString = tmpString.replace("未加标签","");
        if(tmpString.equals(""))
            tmpString = "未加标签";
        final SpannableStringBuilder builder =new SpannableStringBuilder(tmpString);
