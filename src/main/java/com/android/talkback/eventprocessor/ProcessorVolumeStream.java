...
public class ProcessorVolumeStream implements AccessibilityEventListener,
        TalkBackPlusService.KeyEventListener, VolumeButtonPatternDetector.OnPatternMatchListener {
    ...
    private TalkBackPlusService mService;
    ...
    private void handleBothVolumeKeysLongPressed() {
        if (TalkBackPlusService.isServiceActive() && switchTalkBackActiveStateEnabled()) {
    ...
    private void handleSingleTap(int button) {
        if (TalkBackPlusService.isServiceActive() && attemptNavigation(button)) {
            return;
        }
