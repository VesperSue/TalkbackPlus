...
public class AccessibilityTutorialActivity extends Activity implements TutorialNavigationCallback,
    ...
    public void onStart() {
         ...
         * enabled).
         */
        final int serviceState = TalkBackPlusService.getServiceState();
         ...
         */
        if (serviceState == TalkBackPlusService.SERVICE_STATE_SUSPENDED) {
            showAlertDialogAndFinish(R.string.tutorial_service_suspended_title,
                    R.string.tutorial_service_suspended_message);
            return;
        } else if ((serviceState == TalkBackPlusService.SERVICE_STATE_INACTIVE)) {
        ...
        if (preferences.getBoolean(TalkBackPlusService.PREF_FIRST_TIME_USER, true)) {
            final SharedPreferences.Editor editor = preferences.edit();
            editor.putBoolean(TalkBackPlusService.PREF_FIRST_TIME_USER, false);
            editor.apply();
            onFirstTimeLaunch();
        }
    ...
    private void onFirstTimeLaunch() {
        final TalkBackPlusService service = TalkBackPlusService.getInstance();
        if (service == null) {
            return;
        }
